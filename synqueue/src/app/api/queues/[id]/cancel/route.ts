...
export async function POST(
  ...
  if (!queue || !['SERVING', 'WAITING'].includes(queue.status)) return err('Cannot cancel this queue')
  ...
  await prisma.queueLog.create({
    data: { queueId: id, action: 'CANCELLED', userId: session.user.id },
  })

  await updateFrontlinerStats(session.user.id, 'CANCELLED')
  broadcastQueueEvent('QUEUE_UPDATED', { queue: updated }, queue.departmentId, queue.counterId ?? undefined)
  audit('CANCEL', 'Queue', id, session.user.id)
